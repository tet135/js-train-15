...
function generateRandomPassword(length) {
  ...
  // for (const ind in characters) {
  //   const index = Math.floor(Math.random() * 100); //ok
  //   const symbol = characters[index];
    
  //   if (index < characters.length && password.length < length) {
...
function calculateCircleArea(radius) {
  if (typeof radius !== "number") {
    console.log("Error: radius has to be a number.");
    return null;
  } else {
    const circumference = Math.pow(radius, 2) * Math.PI;
...
function calculateHypotenuse(a, b) {
  // return Math.hypot(a, b); //рішення через Math.hypot(a, b)!!!!!!!

  if (typeof a === "number" && typeof b === "number"){
    const hypotenuse = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
    // const hypotenuse = Math.sqrt((a ** 2) + (b ** 2)); рішення через мат.оператори
    return hypotenuse;
  } else {
    console.log("Error");
    return null;
...
function roundObjectValues(obj) {
  if (typeof obj !== "object" || typeof obj === null) {
    ...
    const renewArray = array.map((elem) => {
    ...
    const newObj = Object.fromEntries(renewArray);
    return newObj;
  }
...
function compareAndRound(num1, num2) {
  if (typeof num1 === "number" && typeof num2 === "number") {
    const maxRoundedNumber = Math.round(Math.max(num1, num2));
    return maxRoundedNumber;
  } else {
...
function estimateInvestment(principal, interestRate, years) {
  if (typeof principal === "number" && typeof interestRate === "number" && typeof years === "number") {
   const investment = principal * Math.pow((interestRate + 1), years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (Array.isArray(products)) {
    if (typeof maxPrice === "number") {
      ...
      // }
      const totalPrice = products.reduce((sum, item) => sum += item["price"]);
